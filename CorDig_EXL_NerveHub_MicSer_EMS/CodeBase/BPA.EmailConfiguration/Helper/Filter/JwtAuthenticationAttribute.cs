using BPA.EmailConfiguration;
using BPA.EmailConfiguration.Helper;
using Microsoft.AspNetCore.Mvc.Filters;

namespace MicSer.EmailConfiguration.Helper.Filter
{
        ...
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            ...
            if (headerContent.Count == 1)
            {
                ...
                if (bearer.ToLower().Contains(_bearer))
                {
                    ...
                    var TokenDetail = await CallAPI.HttpClientAsync<BPA.EmailConfiguration.Models.TokenDetail>(HttpMethod.Post, GlobalObject.URLValidateSSO, string.Empty, token);
