...
namespace BPA.Security.DataLayer.ExternalRef
{
    public class DLSBUInfo : IDisposable
    {
        ...
        private const string SQL_SELECT_SBUALLBASEDONCLIENTID = @"select SB.SBUId,ISNULL(Erpid,0) ErpId,CL.CLientID,sb.Name,sb.description,sb.IsClientSBU,sb.disabled,sb.Createdby from Config.tblClientSBUMap(NOLOCK) CL,Config.tblSBUMaster(NOLOCK) sb
                                                            where CL.CLientID=@clientId  and CL.Disabled=0 and
                                                            CL.SBUID=sb.SBUID";
        ...
        //private const string SQL_SELECT_SBU = @"SELECT sbuid,ISNULL(Erpid,0) ErpId,Name,Description,IsClientSBU,Disabled,Createdby FROM Config.tblSBUMaster(NOLOCK) SB ";
        //Bug Id 69 Shishir
        ...
        private const string SQL_DELETE_SBU = @"DELETE Config.tblSBUMaster WHERE SBUId=@SBUId";
        ...
        public void DeleteData(BESBUInfo oSBU)
        {
            try
            {
                //*************************************
                ...
                DbCommand dbDeleteSBUCommand = db.GetSqlStringCommand(SQL_DELETE_SBU);
                db.AddInParameter(dbDeleteSBUCommand, PARAM_SBUID, DbType.Int32, oSBU.iSBUID);
                ...
                using (DbConnection conn = db.CreateConnection())
                {
                    ...
                    using (DbTransaction trans = conn.BeginTransaction()) //Start Transaction
                    {
                        try
                        {
                            db.ExecuteNonQuery(dbDeleteSBUCommand, trans);
        ...
        #region GetShiftList
        /// <summary>
        /// Gets the all Shift data as list.
        /// </summary>
        /// <param name="bGetAll">if set to <c>true</c> [Get all].</param>
        /// <returns>
        /// If Get ALL is true return List of All the Shift
        /// else Return Active Shift List.
        /// </returns>
        public List<BESBUInfo> GetSBUList(bool bGetAll)
        {
            return GetSBUList("", bGetAll);
        }

        /// <summary>
        /// Gets the Matching list of Shift
        /// </summary>
        /// <param name="sName">Name of the Shift.</param>
        /// <param name="bGetAll">if set to <c>true</c> [Get all].</param>
        /// <returns>
        /// If Get ALL is true return List of All the Shift
        /// else Return Active Shift List.
        ...
        /// <returns></returns>
        public List<BESBUInfo> GetSBUListBsedOnClient(int iClientId)
        {
            List<BESBUInfo> lSBU = new List<BESBUInfo>();

            Database db = DL_Shared.dbFactory(_oTenant);

            DbCommand dbCommand = db.GetSqlStringCommand(SQL_SELECT_SBUALLBASEDONCLIENTID);


            db.AddInParameter(dbCommand, PARAM_CLIENTID, DbType.Int16, iClientId);
            using (IDataReader rdr = db.ExecuteReader(dbCommand))
            {
                while (rdr.Read())
                {
        ...
        /// </summary>
        /// <param name="iclientId">The iclient id.</param>
        /// <returns></returns>
        ...
        /// </summary>
        /// <returns></returns>
        public List<BESBUInfo> GetSBUListall()
        {
            List<BESBUInfo> lSBUInfo = new List<BESBUInfo>();
            ...
            DbCommand dbCommand = db.GetSqlStringCommand(SQL_SELECT_SBU);
            db.AddInParameter(dbCommand, PARAM_NAME, DbType.String, "");
